...
contract Caller {
    ...
    function setValueByCall(address callee, uint256 _value)
        public
    ...
    function setValueByDelegateCall(address callee, uint256 _value)
        public
        returns (bool success)
    {
        // bool _success ;
        // bytes memory data;
        // (bool _success,bytes memory  data) =  callee.call(abi.encodeWithSignature("setValue", _value));
        (bool _success, bytes memory _calleeSuccess) = callee.delegatecall(
            abi.encodeWithSignature("setValue(uint256)", _value)
        );
    ...
    function setValueByCallCode(address callee, uint256 _value)
        public
        returns (bool)
    {
    ...
    function delegatedFwd(address _dst, uint256 _value) public {
        bytes memory _calldata = abi.encodeWithSignature(
            "setValue(uint256)",
            _value
        );
